...
(function ($) {
  ...
  function gsapController() {
    ...
    function initImagePreview() {
      ...
      // Zoom function jo scale ko update karega
      const updateZoom = (newScale) => {
        // Zoom limits (0.5x se 5x tak)
    ...
    function galleryModel() {
      ...
      const spImg = document.getElementById("spPopupImg");
      const spClose = document.querySelector(".sp-close-icon");
      const btnNext = document.getElementById("spNextBtn");
      const btnPrev = document.getElementById("spPrevBtn");

      const spGalleryImages = Array.from(document.querySelectorAll(".gallery-item img"));
      let spCurrentIdx = 0;

      // --- NAYA: Image load karne ka naya smooth tarika ---
      const loadNewImage = (index) => {
        // 1. Pehle current image ko fade out karein
        spImg.classList.remove("sp-image-active");

        // 2. Chote se delay ke baad (taaki fade-out dikhe), image badlein
        setTimeout(() => {
          spImg.src = spGalleryImages[index].src;
          spCurrentIdx = index;

          // 3. Jab nayi image load ho jaye (browser mein), use fade-in karein
          spImg.onload = () => {
            spImg.classList.add("sp-image-active");
          };
        }, 250); // Ye delay CSS transition time se kam hona chahiye
      };

      // Click Function (First time open karne ke liye)
      spGalleryImages.forEach((img, index) => {
        img.style.cursor = "zoom-in";
        img.addEventListener("click", () => {
          spModal.style.display = "flex";
          // First time directly load, without timeout, but with fade
          spImg.src = img.src;
          spCurrentIdx = index;
      ...
      const closeSpModal = () => {
        ...
        document.body.style.overflow = "auto";
        // Jab close ho to active class hata dein, taaki next time fir animation ho
        spImg.classList.remove("sp-image-active");
      };

      spClose.onclick = closeSpModal;

      // Next/Prev Logic - Ab loadNewImage use karenge
      const showImgByStep = (step) => {
        let nextIdx = (spCurrentIdx + step + spGalleryImages.length) % spGalleryImages.length;
        loadNewImage(nextIdx);
      };

      btnNext.onclick = (e) => { e.stopPropagation(); showImgByStep(1); };
      btnPrev.onclick = (e) => { e.stopPropagation(); showImgByStep(-1); };

      // Bahar click karne par band
      spModal.onclick = (e) => {
        if (e.target === spModal || e.target.classList.contains('sp-modal-container')) {
          closeSpModal();
        }
      };
      ...
      document.addEventListener("keydown", (e) => {
        if (spModal.style.display === "flex") {
          if (e.key === "ArrowRight") showImgByStep(1);
          if (e.key === "ArrowLeft") showImgByStep(-1);
    ...
    $(".title-highlight").each(function () {
      const highlightText = new SplitText(this, {
        type: "lines",
        linesClass: "line",
      });
      ...
      tl.to($(this).find(".line"), {
        "--highlight-offset": "100%",
        stagger: 0.4,
      });
    });

    $(".title-highlight1").each(function () {
      new SplitText(this, { type: "lines", linesClass: "line" });
